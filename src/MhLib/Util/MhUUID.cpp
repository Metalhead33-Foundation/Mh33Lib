...
void uuid_encode(Util::Buffer& buff) {
	uint8_t i=0;
	for(auto& it : buff) {
		it ^= std::byte{0xAA};
		it ^= std::byte{i};
...
void uuid_decode(Util::Buffer& buff) {
	uint8_t i=0;
	for(auto& it : buff) {
		it ^= std::byte{i};
		it ^= std::byte{0xAA};
...
{
	Io::FixedBufferWrapper buff(__uuid,size);
	Io::DataStream<Util::Endian::Big>(buff)  << uint32_t(rng.generate())
...
{
	Io::FixedBufferWrapper buff(__uuid,size);
	Io::DataStream<Util::Endian::Big>(buff) << int64_t(time.count()) << randomnumA << randomnumB;
...
{
	Io::FixedBufferWrapper buff(__uuid,size);
	Io::DataStream<Util::Endian::Big>(buff) << int64_t(time.count()) << randomnum;
	/*uuid_encode(buff.getBuffer());
	*this = buff.getBuffer();*/
}
...
{
	int64_t tmpTime;
	Io::FixedBufferWrapper buff(const_cast<uint8_t*>(__uuid),size);
	Io::DataStream<Util::Endian::Big>(buff) >> tmpTime >> randomNumA >> randomNumB;
...
void UUID::toTimeAndRandom(std::chrono::milliseconds &time, uint64_t &randomNum) const
{
	int64_t tmpTime;
	Io::FixedBufferWrapper buff(const_cast<uint8_t*>(__uuid),size);
	Io::DataStream<Util::Endian::Big>(buff) >> tmpTime >> randomNum;
	time = std::chrono::milliseconds(tmpTime);
}
...
{
	Io::FixedBufferWrapper buff(const_cast<uint8_t*>(__uuid),size);
	Io::DataStream<Util::Endian::Big>(buff) >> randomNum1 >> randomNum2 >> randomNum3 >> randomNum4;
}
